...
columns = ['Date & Time', 'Period', 'Period Time', 'Period Time Remaining', 'Period Type', 'Game ID', 'Team Name', 'Shot or Goal', 'X-Coordinate', 'Y-Coordinate', 'Shooter Name', 'Goalie Name', 'Scorer Name', 'Shot Type', 'Was Net Empty', 'Goalie Strength']
            ...
            if file[-5:] == '.json':
                ...
                for i in range(len(all_plays)):
                    ...
                    if event == 'Shot' or event == 'Goal':
                        ...
                        row_data = [date_time, period, period_time, period_time_remaining, period_type, game_id, team_name, event, coord_x, coord_y, shooter_name, goalie_name, scorer_name, shot_type, was_net_empty, goalie_strength]

                        data.append(row_data)

            # writing to csv file 
            with open(f'CSV_data/{game_type}/{year}/{game_id}.csv', 'w') as csvfile: 
